...
import { LoadedPlugin, PluginManifest } from '../../shared/types';
...
export class PluginLoader {
   ...
   * 加载插件视图配置
   */
  private async loadPluginViews(pluginPath: string, manifest: PluginManifest): Promise<any[]> {
   ...
   * 创建新插件模板
   */
  async createPluginTemplate(name: string, options?: {
    withPython?: boolean;
    withReact?: boolean;
  }): Promise<string> {
    ...
    // 创建views.json
    fs.writeFileSync(
      path.join(pluginPath, 'views.json'),
      JSON.stringify([
        {
          id: `${pluginId}-main`,
          title: name,
